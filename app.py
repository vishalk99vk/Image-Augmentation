...
def apply_color_filters(img):
    ...
    filters = [
        lambda i: ImageEnhance.Brightness(i).enhance(random.uniform(1.1, 1.3)),  # brighter
        lambda i: ImageEnhance.Brightness(i).enhance(random.uniform(0.7, 0.9)),  # darker
        lambda i: ImageEnhance.Contrast(i).enhance(random.uniform(1.1, 1.3)),    # higher contrast
        lambda i: ImageEnhance.Contrast(i).enhance(random.uniform(0.7, 0.9)),    # lower contrast
        lambda i: i.convert("RGB").point(lambda p: min(255, int(p * 1.05))),    # slight warm tint
        lambda i: i.convert("RGB").point(lambda p: max(0, int(p * 0.95))),      # slight cool tint
    ]
...
def augment_image(img):
    angles = list(range(-10, 11, 2))  # -10 to +10 degrees step 2
    augmented_images = []
...
st.title("Image Augmentation Tool with 3D Rotation, Crop, Filters, and Noise")
...
if uploaded_file:
    ...
    if st.button("Generate Augmented Images"):
        with st.spinner("Generating images..."):
            ...
            # Save all images into a zip buffer
            ...
            st.success(f"Generated {len(augmented_imgs)} augmented images!")
